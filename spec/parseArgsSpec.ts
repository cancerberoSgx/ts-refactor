...
describe('parseArgs', () => {
  ...
  it('should parse files with or without category or fix', () => {
    ...
    expect(parseArgs({_: [CATEGORY.move, './file']})).toEqual({category: CATEGORY.move, files: ['./file'], toolOptions: {}})
    expect(parseArgs({_: [CATEGORY.move, FIX.organizeImports, './file']})).toEqual({category: CATEGORY.move, fix: FIX.organizeImports, files: ['./file'], toolOptions: {}})
  });
  ...
  it('should parse tool options with or without files, category or fix', () => {
    ...
    expect(parseArgs({_: [CATEGORY.fix], help: 'help', tsConfigPath: 'foo/tsconfig.json'})).toEqual({category: CATEGORY.fix, toolOptions: {help: true, tsConfigPath: 'foo/tsconfig.json'}, files: []})
  });
