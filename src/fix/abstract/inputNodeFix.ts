...
export abstract class InputNodeFix<T extends Node> extends DestFileFix<InputNodeFixOptions<T>> implements IInputFileFix<T>{
 ...
 failOnInputNodeNotFound: boolean = true
  ...
  async inquireOptions(options: InputNodeFixOptions<T>) :Promise< InputNodeFixOptions<T>>{
    ...
    if (inputNodes.length===0 && !(options.options.toolOptions && options.options.toolOptions.dontAsk)) {
      const thisOptions = await prompt<{ nodes: T[] }>([ // TODO: support multiple input nodes
        {
          type: 'list',
          name: 'nodes',
